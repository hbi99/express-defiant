{
	...
	init: function() {
		var self = debug.left,
			textarea = $('section.xslt-view textarea')[0],
			allXslt = textarea.value,
			active = allXslt.match(/<!-- ACTIVE: (.+?) -->/i)[1],
			rx = new RegExp('<!-- START: '+ active +'.xslt? -->([\\s\\S]*?)<!-- END: '+ active +'.xslt? -->', 'mi'),
			templName;

		this.el = debug.body.find('.xslt-view');
		this.allXslt = allXslt;

		// template contents
		textarea.value = this.allXslt.match(rx)[1].trim();

		// template name
		rx = new RegExp('<!-- START: ('+ active +'.xslt?) -->', 'mi');
		templName = this.allXslt.match(rx)[1].trim();
		this.el.find('.tabbar .active').html(templName);
		...
		switch(cmd) {
  			...
  			case 'cursorActivity':
				editor = self.editor;
	  			...
	  			if (line.match(/select="(.+?)"/i) === null) return;
